...
PBTVMF4=function(x,alpha)
{
  ...
  N=sum(sequence_of_n)
  A1=a[1]*sqrt(sum(y[[1]]**2))+a[2]*sqrt(sum(y[[2]]**2))+a[3]*sqrt(sum(y[[3]]**2))+a[4]*sqrt(sum(y[[4]]**2))
  ...
  for(h in 1:H)
  {
    ...
    B1=b[1]*sqrt(sum(z[[1]]**2))+b[2]*sqrt(sum(z[[2]]**2))+b[3]*sqrt(sum(z[[3]]**2))+b[4]*sqrt(sum(z[[4]]**2))
