...
PBTVMF5=function(x,alpha)
{
  ...
  N=sum(sequence_of_n)
  A1=a[1]*sqrt(sum(y[[1]]**2))+a[2]*sqrt(sum(y[[2]]**2))+a[3]*sqrt(sum(y[[3]]**2))+a[4]*sqrt(sum(y[[4]]**2))+a[5]*sqrt(sum(y[[5]]**2))
  ...
  for(h in 1:H)
  {
    ...
    z1=(z[[1]]+z[[2]]+z[[3]]+z[[4]]+z[[5]])
    mu_hat1=z1/sqrt(sum(z1**2))
    B1=b[1]*sqrt(sum(z[[1]]**2))+b[2]*sqrt(sum(z[[2]]**2))+b[3]*sqrt(sum(z[[3]]**2))+b[4]*sqrt(sum(z[[4]]**2))+b[5]*sqrt(sum(z[[5]]**2))
